...
# NOTE: emotionality trends are no longer needed with our project reframing, and only use VADER
# We keep the code here in case we do want to go back and add anything emotionality-wise later
...
def plot_ideology_trends(df: pd.DataFrame) -> str:
    ...
    ax.set_ylim(-1.1, 1.1)
    ax.set_xlim(pd.Timestamp(f"{YEAR_RANGE[0]}-01-01"), pd.Timestamp(f"{YEAR_RANGE[1]}-12-31"))
...
def generate_all_plots(df: pd.DataFrame) -> list:
    ...
    # Filter to 2013-2022 window only — removes any date parsing artifacts
    df = df[
        (df["date"].dt.year >= YEAR_RANGE[0]) &
        (df["date"].dt.year <= YEAR_RANGE[1])
    ].copy()
    print(f"  Filtered to {YEAR_RANGE[0]}–{YEAR_RANGE[1]}: {len(df)} headlines")
