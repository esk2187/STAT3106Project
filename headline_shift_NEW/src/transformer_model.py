...
Also provides:
  ...
  - MultiTaskRoBERTaClassifier: dual-head RoBERTa (ideology + emotionality)    — extended comparison

Hyperparameters (tuned via validation set search):
  LR = 3e-5, loss weights = ideology:2.0 / emotionality:0.5
"""
...
ROBERTA_MULTITASK_DIR = os.path.join(MODELS_DIR, "roberta_multitask")
...
class MultiTaskClassifier:
    ...
    def train(
        ...
        epochs: int = FAST_EPOCHS,
        batch_size: int = FAST_BATCH,
        lr: float = 3e-5,  # tuned via validation set search
    ):
        """
        Fine-tune the model.
        ...
        for epoch in range(1, epochs + 1):
            ...
            for batch in pbar:
                ...
                if use_emotion:
                    ...
                    loss = 2.0 * loss + 0.5 * bce_loss(em_pred, em_labels)
...
class RoBERTaClassifier:
    ...
    def train(self, train_df, val_df=None, epochs=FAST_EPOCHS, batch_size=FAST_BATCH, lr=3e-5):
        train_loader = self._make_loader(
            train_df["headline"].tolist(), train_df["label_id"].tolist(),
            batch_size, shuffle=True,
        )
...
# ── Dual-head RoBERTa (ideology + emotionality) ────────────────────────────

class MultiTaskRoBERTa(nn.Module):
    """
    RoBERTa backbone with two task-specific heads:
      - Ideology head:     768 → 256 → 3  (left / center / right)
      - Emotionality head: 768 → 128 → 1  (sigmoid, 0-1 score)
    Same architecture as MultiTaskDistilBERT but with RoBERTa backbone.
    """

    def __init__(self, model_name=ROBERTA_MODEL_NAME, dropout=0.1):
        super().__init__()
...
class MultiTaskRoBERTaClassifier:
    """
    Dual-head RoBERTa: jointly predicts ideology (3-class) + emotionality (scalar).
    Same training paradigm as MultiTaskClassifier but with RoBERTa backbone.
    LR=3e-5, loss weights ideology:2.0 / emotionality:0.5 (matched to DistilBERT tuned config).
    """

    def __init__(self, model_name=ROBERTA_MODEL_NAME, max_len=FAST_MAX_LEN):
        self.tokenizer = RobertaTokenizer.from_pretrained(model_name)
        self.model = MultiTaskRoBERTa(model_name)
        self.model.to(DEVICE)
        self.max_len = max_len
    ...
    def train(
        ...
        epochs: int = FAST_EPOCHS,
        batch_size: int = FAST_BATCH,
        lr: float = 3e-5,  # matched to DistilBERT tuned LR
    ):
        use_emotion = emotion_df is not None and len(emotion_df) >= 2
        if not use_emotion:
            print("  ⚠  No emotionality labels — training ideology-only.")
        ...
        for epoch in range(1, epochs + 1):
            ...
            for batch in pbar:
                ...
                ideology_logits, _ = self.model(input_ids, attention_mask)
                loss = ce_loss(ideology_logits, ideology_labels)
                ...
                if use_emotion:
                    ...
                    em_labels = em_batch["emotion_label"].to(DEVICE)
                    _, em_pred = self.model(em_ids, em_mask)
                    # Same loss weights as DistilBERT tuned config: ideology:2.0, emotionality:0.5
                    loss = 2.0 * loss + 0.5 * bce_loss(em_pred, em_labels)
    ...
    def evaluate_emotionality(self, emotion_df: pd.DataFrame, batch_size=64) -> dict:
        scores = self.predict_emotionality(emotion_df["headline"].tolist(), batch_size)
        y_true_cont = emotion_df["emotionality_score"].values
        threshold = np.mean(y_true_cont)  # mean threshold for win-rate labels
        y_true_bin = (y_true_cont >= threshold).astype(int)
        y_pred_bin = (scores >= 0.5).astype(int)
        f1 = f1_score(y_true_bin, y_pred_bin, zero_division=0)
        try:
    ...
    def save(self, path=ROBERTA_MULTITASK_DIR):
        os.makedirs(path, exist_ok=True)
        torch.save(self.model.state_dict(), os.path.join(path, "model.pt"))
        self.tokenizer.save_pretrained(path)
        print(f"  Saved dual-head RoBERTa model → {path}")

    @classmethod
    def load(cls, path=ROBERTA_MULTITASK_DIR, model_name=ROBERTA_MODEL_NAME):
        obj = cls.__new__(cls)
        obj.tokenizer = RobertaTokenizer.from_pretrained(path)
        obj.model = MultiTaskRoBERTa(model_name)
        obj.model.load_state_dict(torch.load(os.path.join(path, "model.pt"), map_location=DEVICE))
        obj.model.to(DEVICE)
        obj.model.eval()
        obj.max_len = FAST_MAX_LEN
        return obj


def train_roberta_multitask(train_df, val_df, test_df, emotion_df=None, epochs=3, batch_size=32):
    """Convenience: train dual-head RoBERTa, print results, save. Returns clf + metrics."""
    print("\n══ RoBERTa Dual-Head (ideology + emotionality) ══")
    print(f"  Device: {DEVICE}")
    print(f"  Train: {len(train_df)} | Val: {len(val_df)} | Test: {len(test_df)}")
    if emotion_df is not None:
        print(f"  Emotionality labels: {len(emotion_df)}")

    clf = MultiTaskRoBERTaClassifier()
    clf.train(train_df, val_df, emotion_df=emotion_df, epochs=epochs, batch_size=batch_size)
